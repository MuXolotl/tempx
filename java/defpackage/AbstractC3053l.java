...
/* JADX INFO: renamed from: lؔۦۛ, reason: invalid class name and case insensitive filesystem */
...
public abstract class AbstractC3053l {
    ...
    public static ColorStateList loadAd(Resources resources, XmlPullParser xmlPullParser, AttributeSet attributeSet, Resources.Theme theme) throws XmlPullParserException, IOException {
        int depth;
        int color;
        float f;
        int iIsPro;
        ...
        while (true) {
            ...
            if (next == 2 && depth <= depth2 && xmlPullParser.getName().equals("item")) {
                int[] iArr2 = AbstractC6368l.yandex;
                ...
                if (resourceId != -1) {
                    ...
                    if (i3 < 28 || i3 > 31) {
                        try {
                            color = yandex(resources, resources.getXml(resourceId), theme).getDefaultColor();
                ...
                if (f != 1.0f || z) {
                    int iCrashlytics = AbstractC12704l.crashlytics((int) ((Color.alpha(color) * f) + 0.5f), 0, 255);
                    if (z) {
                        C4710l c4710lYandex = C4710l.yandex(color);
                        float f4 = c4710lYandex.yandex;
                        float f5 = c4710lYandex.loadAd;
                        ...
                        if (f5 >= 1.0d && Math.round(f2) > 0.0d && Math.round(f2) < 100.0d) {
                            ...
                            while (true) {
                                if (Math.abs(f6 - f5) < 0.4f) {
                                    ...
                                    depth2 = depth2;
                                    if (c4710l != null) {
                                        iIsPro = c4710l.crashlytics(c6912l);
                                        break;
                                    }
                                    iIsPro = AbstractC9464l.isPro(f2);
                                    break;
                                }
                                float f8 = 1000.0f;
                                float f9 = f3;
                                ...
                                while (true) {
                                    ...
                                    iArrTrimStateSet = iArrTrimStateSet;
                                    int iCrashlytics2 = C4710l.loadAd(f12, f7, fMin).crashlytics(C6912l.firebase);
                                    float fRemoteconfig = AbstractC9464l.remoteconfig(Color.red(iCrashlytics2));
                                    float fRemoteconfig2 = AbstractC9464l.remoteconfig(Color.green(iCrashlytics2));
                                    float fRemoteconfig3 = AbstractC9464l.remoteconfig(Color.blue(iCrashlytics2));
                                    float[] fArr = AbstractC9464l.amazon[r16 == true ? 1 : 0];
                                    float f13 = ((fRemoteconfig3 * fArr[2]) + ((fRemoteconfig2 * fArr[r16 == true ? 1 : 0]) + (fRemoteconfig * fArr[0]))) / f3;
                                    float fCbrt = f13 <= 0.008856452f ? f13 * 903.2963f : (((float) Math.cbrt(f13)) * 116.0f) - 16.0f;
                                    float fAbs = Math.abs(f2 - fCbrt);
                                    if (fAbs < 0.2f) {
                                        C4710l c4710lYandex2 = C4710l.yandex(iCrashlytics2);
                                        C4710l c4710lLoadAd = C4710l.loadAd(c4710lYandex2.crashlytics, c4710lYandex2.loadAd, fMin);
                                        float f14 = c4710lYandex2.amazon - c4710lLoadAd.amazon;
                                        float f15 = c4710lYandex2.purchase - c4710lLoadAd.purchase;
                                        float f16 = c4710lYandex2.billing - c4710lLoadAd.billing;
                                        depth2 = depth2;
                                        float fPow = (float) (Math.pow(Math.sqrt((f16 * f16) + (f15 * f15) + (f14 * f14)), 0.63d) * 1.41d);
                                        if (fPow <= 1.0f) {
                                            f11 = fPow;
                                            f8 = fAbs;
                                            c4710l2 = c4710lYandex2;
                                        }
                                    } else {
                                        depth2 = depth2;
                                    }
                                ...
                                if (!z2) {
                                    ...
                                    if (c4710l3 != null) {
                                        iIsPro = c4710l3.crashlytics(c6912l);
                                        break;
                                    }
                                    f7 = ((f5 - f6) / 2.0f) + f6;
                                    z2 = false;
                    ...
                    color = (16777215 & color) | (iCrashlytics << 24);
                } else {
                    iArrTrimStateSet = iArrTrimStateSet;
                    depth2 = depth2;
                }
    ...
    public static ColorStateList yandex(Resources resources, XmlResourceParser xmlResourceParser, Resources.Theme theme) {
        int next;
        ...
        if (next == 2) {
            return loadAd(resources, xmlResourceParser, attributeSetAsAttributeSet, theme);
        }
