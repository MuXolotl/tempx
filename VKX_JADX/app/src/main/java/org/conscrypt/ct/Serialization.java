...
public class Serialization {
    ...
    public static byte[] readDEROctetString(InputStream inputStream) throws SerializationException {
        ...
        if (i != 4) {
            throw new SerializationException(AbstractC0653l.vip(i, "Wrong DER tag, expected OCTET STRING, got "));
        }
    ...
    public static long readLong(InputStream inputStream, int i) {
        if (i > 8 || i < 0) {
            C8339l.metrica(AbstractC0653l.vip(i, "Invalid width: "));
            return 0L;
        }
        long j = 0;
    ...
    public static int readNumber(InputStream inputStream, int i) throws SerializationException {
        if (i > 4 || i < 0) {
            throw new SerializationException(AbstractC0653l.vip(i, "Invalid width: "));
        }
        int i2 = 0;
    ...
    public static void writeNumber(OutputStream outputStream, long j, int i) {
        if (i < 0) {
            throw new SerializationException(AbstractC0653l.vip(i, "Negative width: "));
        }
        if (i < 8 && j >= (1 << (i * 8))) {
