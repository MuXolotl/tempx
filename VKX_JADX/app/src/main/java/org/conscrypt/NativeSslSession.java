...
abstract class NativeSslSession {
    ...
    private static void checkRemaining(ByteBuffer byteBuffer, int i) throws IOException {
        if (i < 0) {
            C18262l.metrica(AbstractC0653l.vip(i, "Length is negative: "));
        } else {
            if (i <= byteBuffer.remaining()) {
                return;
            }
            StringBuilder sbSignature = AbstractC2812l.Signature("Length of blob is longer than available: ", i, " > ");
            sbSignature.append(byteBuffer.remaining());
