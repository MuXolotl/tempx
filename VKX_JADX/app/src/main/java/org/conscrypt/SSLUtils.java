...
final class SSLUtils {
    ...
    public static String[] decodeProtocols(byte[] bArr) {
        ...
        while (i2 < bArr.length) {
            ...
            if (b < 0 || b > bArr.length - i2) {
                StringBuilder sbSubscription = AbstractC14814l.subscription(b, i2, "Protocol has invalid length (", " at position ", "): ");
                if (bArr.length < 50) {
                    strRemoteconfig = Arrays.toString(bArr);
                } else {
                    strRemoteconfig = AbstractC14814l.remoteconfig(bArr.length, " byte array", new StringBuilder());
                }
                sbSubscription.append(strRemoteconfig);
    ...
    public static byte[] encodeProtocols(String[] strArr) {
        if (strArr == null) {
            C8339l.metrica("protocols array must be non-null");
            return null;
        }
        if (strArr.length == 0) {
        ...
        for (int i2 = 0; i2 < strArr.length; i2++) {
            ...
            if (str == null) {
                C8339l.metrica(AbstractC15560l.tapsense("protocol[", i2, "] is null"));
                return null;
            }
            int length = str.length();
            if (length == 0 || length > MAX_PROTOCOL_LENGTH) {
                C8339l.metrica(AbstractC12589l.premium(i2, length, "protocol[", "] has invalid length: "));
                return null;
            }
            i += length + 1;
        }
