...
public abstract class BaseKeyFactorySpi extends KeyFactorySpi implements AsymmetricKeyInfoConverter {
    ...
    public PrivateKey engineGeneratePrivate(KeySpec keySpec) throws InvalidKeySpecException {
        if (!(keySpec instanceof PKCS8EncodedKeySpec)) {
            C18073l.subscription("key spec not recognized");
            return null;
        }
        try {
            return generatePrivate(C2756l.vip(((PKCS8EncodedKeySpec) keySpec).getEncoded()));
        } catch (Exception e) {
            throw new InvalidKeySpecException(AbstractC12900l.admob(e, new StringBuilder("encoded key spec not recognized: ")));
        }
    }
