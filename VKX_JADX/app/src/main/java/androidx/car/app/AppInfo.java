...
public final class AppInfo {
    ...
    public static AppInfo create(Context context) {
        ...
        if (iRetrieveMinCarAppApiLevel >= 1 && iRetrieveMinCarAppApiLevel <= AbstractC16181l.loadAd()) {
            return new AppInfo(iRetrieveMinCarAppApiLevel, AbstractC16181l.loadAd(), context.getResources().getString(R.string.car_app_library_version));
        }
        StringBuilder sbSignature = AbstractC2812l.Signature("Min API level (androidx.car.app.minCarApiLevel=", iRetrieveMinCarAppApiLevel, ") is out of range (1-");
        sbSignature.append(AbstractC16181l.loadAd());
        sbSignature.append(")");
    ...
    public static int retrieveMinCarAppApiLevel(Context context) {
        try {
            ...
            C8339l.metrica("Unable to read min API level from manifest");
            return 0;
        }
    }
