package bruhcollective.itaysonlab.libvkmusic.objects;

import defpackage.AbstractC12589l;
import defpackage.AbstractC15639l;
import defpackage.AbstractC16901l;
import defpackage.AbstractC2812l;
import defpackage.AbstractC7188l;
import defpackage.C0396l;
import defpackage.C0458l;
import defpackage.C11467l;
import defpackage.C12952l;
import defpackage.C18262l;
import defpackage.C3631l;
import defpackage.C5746l;
import defpackage.C8413l;
import java.io.EOFException;
import java.util.List;
import java.util.Set;
import kotlin.Metadata;

/* JADX INFO: compiled from: r8-map-id-7761d6fd52750600b6eeb3da7b88ede05d5cd0fcaae415e0ed7a2830c95bc1fb */
/* JADX INFO: loaded from: classes.dex */
@Metadata(k = 3, mv = {2, 2, 0}, xi = 48)
public final class UmaTagJsonAdapter extends AbstractC7188l {
    public final AbstractC7188l amazon;
    public final AbstractC7188l crashlytics;
    public final AbstractC7188l loadAd;
    public final AbstractC7188l purchase;
    public final C0458l yandex = C0458l.m547strictfp("name", "apiId", "cover", "relevantArtistsNames");

    public UmaTagJsonAdapter(C12952l c12952l) {
        C5746l c5746l = C5746l.f12138l;
        this.loadAd = c12952l.loadAd(String.class, c5746l, "name");
        this.crashlytics = c12952l.loadAd(Long.TYPE, c5746l, "apiId");
        this.amazon = c12952l.loadAd(UmaCover.class, c5746l, "cover");
        this.purchase = c12952l.loadAd(AbstractC15639l.mopub(List.class, String.class), c5746l, "relevantArtistsNames");
    }

    @Override // defpackage.AbstractC7188l
    public final void billing(C3631l c3631l, Object obj) {
        if (obj == null) {
            C18262l.mopub();
            return;
        }
        UmaTag umaTag = (UmaTag) obj;
        c3631l.mopub();
        c3631l.isVip("name");
        this.loadAd.billing(c3631l, umaTag.yandex);
        c3631l.isVip("apiId");
        AbstractC12589l.m3427throws(umaTag.loadAd, this.crashlytics, c3631l, "cover");
        this.amazon.billing(c3631l, umaTag.crashlytics);
        c3631l.isVip("relevantArtistsNames");
        this.purchase.billing(c3631l, umaTag.amazon);
        c3631l.adcel();
    }

    @Override // defpackage.AbstractC7188l
    public final Object loadAd(C0396l c0396l) throws C8413l, EOFException {
        c0396l.billing();
        Set setLicense = C5746l.f12138l;
        boolean z = false;
        byte b = -1;
        boolean z2 = false;
        String str = null;
        Long l = null;
        boolean z3 = false;
        UmaCover umaCover = null;
        Object objLoadAd = null;
        while (c0396l.m504package()) {
            int iM503l = c0396l.m503l(this.yandex);
            if (iM503l == -1) {
                c0396l.m502l();
                c0396l.m501l();
            } else if (iM503l == 0) {
                Object objLoadAd2 = this.loadAd.loadAd(c0396l);
                if (objLoadAd2 == null) {
                    setLicense = AbstractC2812l.isVip("name", "name", c0396l, setLicense);
                    z = true;
                } else {
                    str = (String) objLoadAd2;
                }
            } else if (iM503l == 1) {
                Object objLoadAd3 = this.crashlytics.loadAd(c0396l);
                if (objLoadAd3 == null) {
                    setLicense = AbstractC2812l.isVip("apiId", "apiId", c0396l, setLicense);
                    z2 = true;
                } else {
                    l = (Long) objLoadAd3;
                }
            } else if (iM503l == 2) {
                Object objLoadAd4 = this.amazon.loadAd(c0396l);
                if (objLoadAd4 == null) {
                    setLicense = AbstractC2812l.isVip("cover", "cover", c0396l, setLicense);
                    z3 = true;
                } else {
                    umaCover = (UmaCover) objLoadAd4;
                }
            } else if (iM503l == 3) {
                objLoadAd = this.purchase.loadAd(c0396l);
                b = -9;
            }
        }
        c0396l.ads();
        if ((!z) & (str == null)) {
            setLicense = AbstractC2812l.license("name", "name", c0396l, setLicense);
        }
        if ((!z2) & (l == null)) {
            setLicense = AbstractC2812l.license("apiId", "apiId", c0396l, setLicense);
        }
        if ((!z3) & (umaCover == null)) {
            setLicense = AbstractC2812l.license("cover", "cover", c0396l, setLicense);
        }
        if (setLicense.size() != 0) {
            C11467l.subscription(AbstractC16901l.m4210case(setLicense, "\n", null, null, null, 62));
            return null;
        }
        long jLongValue = l.longValue();
        List list = (List) objLoadAd;
        if (b == -9) {
            return new UmaTag(str, jLongValue, umaCover, list);
        }
        if ((b & 8) != 0) {
            list = null;
        }
        return new UmaTag(str, jLongValue, umaCover, list);
    }

    public final String toString() {
        return "GeneratedJsonAdapter(UmaTag)";
    }
}
