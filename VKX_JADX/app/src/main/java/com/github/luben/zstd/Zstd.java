...
public class Zstd {
    ...
    private static int calculateContentSizeAndFrames(byte[] bArr, List<FrameData> list) {
        ...
        while (i < bArr.length) {
            ...
            if (j2 > bArr.length - i) {
                C18073l.license("Invalid compressed size");
                return 0;
            }
            long j3 = frameData.contentSize;
            long j4 = MAX_DECOMPRESS_SIZE;
            if (j3 > j4) {
                C18073l.license("Frame content size is too large");
                return 0;
            }
            i += (int) j2;
            j += j3;
            if (j > j4) {
                C18073l.license("Content size too large");
                return 0;
            }
        }
        return (int) j;
    ...
    public static byte[] decompress(byte[] bArr) {
        ...
        while (i3 < arrayList.size()) {
            ...
            if (jDecompressByteArray != j) {
                C8339l.smaato("decompressed size mismatch");
                return null;
            }
    ...
    public static ByteBuffer getArrayBackedBuffer(BufferPool bufferPool, int i) throws ZstdIOException {
        ...
        if (byteBuffer == null) {
            throw new ZstdIOException(errMemoryAllocation(), AbstractC15560l.tapsense("Cannot get ByteBuffer of size ", i, " from the BufferPool"));
        }
        if (byteBuffer.hasArray() && byteBuffer.arrayOffset() == 0) {
            return byteBuffer;
        }
        bufferPool.release(byteBuffer);
        C8339l.metrica("provided ByteBuffer lacks array or has non-zero arrayOffset");
        return null;
    }
